...
class MultiArmedBandit:
    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        for t in range(steps):
            ...
            if src.random.rand() < self.epsilon:  # Exploration
                action = src.random.randint(0, n_actions)
            else:  # Exploitation
                max_value = np.max(self.Q)
                best_actions = np.where(self.Q == max_value)[0]
                action = src.random.choice(best_actions)  # Random tie-breaking
            ...
            if (t + 1) % s == 0 or t == steps - 1:
                avg_rewards[current_bin] = reward_sum / reward_count
                reward_sum = 0
                reward_count = 0
                current_bin += 1
        ...
        # Return Q-values replicated for each state
